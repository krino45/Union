...
public class RoomsController : ControllerBase
{
    ...
    public async Task<ActionResult<RoomDto>> Update(Guid id, [FromBody] UpdateRoomRequest req, CancellationToken ct)
        => Ok(await _mediator.Send(new UpdateRoomCommand(id, req.BuildingId, req.Number, req.RoomType, req.Capacity, req.HasProjector, req.HasComputers, req.HasLab, req.IsOnline, req.Floor, req.DistanceFromStairsMeters), ct));
...
public record UpdateRoomRequest(Guid BuildingId, string Number, RoomType RoomType, int Capacity, bool HasProjector, bool HasComputers, bool HasLab, bool IsOnline, int Floor = 1, int DistanceFromStairsMeters = 0);
